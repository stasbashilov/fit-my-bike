    ...
    <div class="mx-auto max-w-form">
      ...
      <div class="space-y-4">
        ...
        <BaseCard title="Saddle Height">
          <p class="text-h1 text-accent-600 dark:text-accent-400">
            {{ result?.saddleHeight }} mm
          </p>
          <p class="mt-1 text-small text-neutral-500 dark:text-neutral-400">
            From bottom bracket center to top of saddle
          </p>
          <p
            v-if="result?.saddleHeightNote"
            class="mt-3 rounded-md border border-neutral-200 bg-neutral-100 p-3 text-small text-neutral-700 dark:border-neutral-600 dark:bg-neutral-700 dark:text-neutral-200"
          >
        ...
        <BaseCard title="Body Position">
          <p class="text-small text-neutral-500 dark:text-neutral-400">
            Target torso angle from horizontal
          </p>
          <p class="mt-1 text-h2 text-neutral-900 dark:text-neutral-50">
            {{ result?.torsoAngleMin }}&deg;&ndash;{{ result?.torsoAngleMax }}&deg;
          </p>
          <p
            v-if="result?.torsoAngleNote"
            class="mt-1 text-small text-neutral-500 italic dark:text-neutral-400"
          >
            {{ result.torsoAngleNote }}
          </p>
          <p class="mt-3 text-center text-label text-neutral-400 dark:text-neutral-500">
        ...
        <BaseCard v-if="result?.handlebarWidth !== undefined" title="Handlebar Width">
          <p class="text-h2 text-neutral-900 dark:text-neutral-50">
            {{ result.handlebarWidth }} mm
          </p>
          <p
            v-if="result.handlebarWidthNote"
            class="mt-1 text-small text-neutral-500 dark:text-neutral-400"
          >
        ...
        <BaseCard title="Model Limitations">
          ...
          <ul class="mt-3 space-y-1" aria-label="Model limitations">
            <li
              ...
              :key="index"
              class="flex gap-2 text-small text-neutral-500 dark:text-neutral-400"
            >
              <span aria-hidden="true" class="mt-px shrink-0 text-neutral-400 dark:text-neutral-500">&bull;</span>
              {{ limitation }}
            </li>
          </ul>
        </BaseCard>
