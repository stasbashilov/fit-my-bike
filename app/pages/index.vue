...
<template>
  <main class="min-h-screen bg-neutral-50 px-4 py-10 sm:py-16">
    <div class="mx-auto max-w-form">
      ...
      <!-- Tabs ────────────────────────────────────────────────────────────── -->
      <div class="mb-6 flex rounded-lg border border-neutral-200 bg-white p-1">
        <button
          type="button"
      ...
      <div v-show="activeTab === 'fit'">
        <BaseCard>
          <form novalidate @submit.prevent="onFitSubmit">
            ...
            <div class="mt-8">
              <p v-if="fitSubmitError" role="alert" class="mb-3 text-small text-danger-700">
                {{ fitSubmitError }}
              </p>
      ...
      <div v-show="activeTab === 'pressure'">
        <BaseCard>
          <form novalidate @submit.prevent="onPressureSubmit">
            <div class="space-y-5">
              ...
              <div class="flex flex-col gap-1.5">
                <div class="flex items-center justify-between">
                  <span class="text-label font-medium text-neutral-700">
                    Tire Width <span class="text-danger-500" aria-hidden="true">*</span>
                  </span>
                  <div class="flex overflow-hidden rounded-md border border-neutral-200 text-small">
                    <button
                      type="button"
                      class="px-2.5 py-0.5 transition-colors"
                      :class="tireWidthUnit === 'mm' ? 'bg-accent-500 text-white' : 'bg-white text-neutral-500 hover:text-neutral-700'"
                      @click="onTireWidthUnitChange('mm')"
                    >mm</button>
                    <button
                      type="button"
                      class="px-2.5 py-0.5 transition-colors"
                      :class="tireWidthUnit === 'in' ? 'bg-accent-500 text-white' : 'bg-white text-neutral-500 hover:text-neutral-700'"
                ...
                <div class="relative">
                  <input
                    ...
                    class="w-full rounded-md border bg-white px-3 py-2.5 pr-12 text-body text-neutral-900 placeholder:text-neutral-400 transition-colors focus:outline-none focus:ring-2 focus:ring-accent-500/40"
                    :class="pressureErrors.tireWidthMm ? 'border-danger-500 focus:border-danger-500' : 'border-neutral-300 focus:border-accent-500'"
                  ...
                  <input
                    ...
                    placeholder="e.g. 2.1"
                    :aria-invalid="Boolean(pressureErrors.tireWidthMm)"
                    class="w-full rounded-md border bg-white px-3 py-2.5 pr-10 text-body text-neutral-900 placeholder:text-neutral-400 transition-colors focus:outline-none focus:ring-2 focus:ring-accent-500/40"
                    :class="pressureErrors.tireWidthMm ? 'border-danger-500 focus:border-danger-500' : 'border-neutral-300 focus:border-accent-500'"
                    @input="onTireWidthInchesInput"
                  >
                  <span class="pointer-events-none absolute inset-y-0 right-3 flex items-center text-small text-neutral-500">
                    {{ tireWidthUnit }}
                  </span>
                </div>

                <p class="text-label text-neutral-400">
                ...
                <p v-if="pressureErrors.tireWidthMm" class="text-label text-danger-700">
                  {{ pressureErrors.tireWidthMm }}
                </p>
              </div>
            ...
            <div class="mt-8">
              <p v-if="pressureSubmitError" role="alert" class="mb-3 text-small text-danger-700">
                {{ pressureSubmitError }}
              </p>
        ...
        <div v-if="pressureResult" class="mt-6">
          <BaseCard>
            ...
            <div class="mb-5 border-b border-neutral-100 pb-4">
              ...
              <h2 class="text-h2 text-neutral-900">
            ...
            <div class="grid grid-cols-2 gap-4">
              <div class="rounded-lg bg-neutral-50 px-4 py-5 text-center">
                <p class="text-small text-neutral-400">Front</p>
                <p class="mt-1 text-h1 text-accent-600">
                  {{ pressureResult.frontBar }}<span class="ml-1 text-h2 font-normal">bar</span>
                </p>
                <p class="mt-0.5 text-small text-neutral-500">{{ pressureResult.frontPsi }} psi</p>
              </div>
              <div class="rounded-lg bg-neutral-50 px-4 py-5 text-center">
                ...
                <p class="mt-0.5 text-small text-neutral-500">{{ pressureResult.rearPsi }} psi</p>
              </div>
            </div>

            <!-- Note -->
            <p class="mt-5 text-small text-neutral-600">
              {{ pressureResult.note }}
            </p>
            ...
            <ul class="mt-4 space-y-1">
              <li
                ...
                :key="limitation"
                class="flex items-start gap-2 text-small text-neutral-400"
              >
                <span class="mt-0.5 text-neutral-300" aria-hidden="true">•</span>
                {{ limitation }}
              </li>
            </ul>
          </BaseCard>
