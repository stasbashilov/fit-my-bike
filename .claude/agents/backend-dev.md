...
- Fit calculation business logic (saddle height, handlebar reach/angle, etc.) — pure functions in `server/utils/fit-calculations.ts`, no side effects, easily unit-testable. **The source of formulas and discipline-specific coefficients is `docs/FIT-METHODOLOGY.md`. Do not invent your own coefficients — if the methodology doesn't cover a case, explicitly flag it in your report as an open question.**
